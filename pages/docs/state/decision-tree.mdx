---
description: React state management guide - when to use useState, useReducer, Zustand, TanStack Query, or Context. Decision tree for choosing the right state solution.
---
...
<Callout type="info"><strong>Golden Rule:</strong> Server data → TanStack Query, Global client state → Zustand, Local UI → useState</Callout>
...
<Callout type="error" emoji="❌"><strong>Don't store server data in Zustand.</strong> Use TanStack Query for automatic caching, refetching, and deduplication.</Callout>
...
<Callout type="warning"><strong>Don't duplicate state.</strong> Derive values from source of truth using <code>useMemo</code>.</Callout>
